...
export const ProjectDetails: React.FC = () => {
  ...
  if (!project) return <div className="p-20 text-center font-black uppercase text-slate-500 tracking-widest">{t('loading')}</div>;
  ...
  const handlePostComment = () => {
    ...
    const newComment = { id: Date.now().toString(), airdropId: project.id, username: user.username || 'Hunter', address: user.address, avatar: user.avatar, content: commentText, rating: userRating, createdAt: new Date().toLocaleString(), createdAtTimestamp: now, isApproved: false };
    
    setComments(prev => [...prev, newComment]);
    
    // Update local activity tracking (independent of comment existence)
    const updatedUser = { 
      ...user, 
      lastCommentTimestamps: { ...projectTimestamps, [project.id]: now } 
    };
    setUsersList(prev => prev.map(u => u.address === user.address ? updatedUser : u));
  ...
  const handleSuggestGuide = () => {
    ...
    const newGuide: Guide = { id: Date.now().toString(), platform: guideData.platform, author: guideData.author, url: guideData.url, lang: guideData.countryCode === 'us' ? 'en' : 'tr', countryCode: guideData.countryCode, isApproved: false, airdropId: project.id, createdAt: Date.now() };
            ...
            <div className="flex flex-col md:flex-row md:items-center gap-6 relative z-10">
              ...
              <div className="flex-1">
                ...
                <div className="flex items-center gap-3 mt-6">
                   {project.socials?.website && <a href={project.socials.website} target="_blank" className="p-2 bg-slate-50 dark:bg-slate-800 rounded-xl hover:text-primary-600 transition-all text-slate-400 shadow-sm"><Globe size={16} /></a>}
                   {project.socials?.twitter && <a href={project.socials.twitter} target="_blank" className="p-2 bg-slate-50 dark:bg-slate-800 rounded-xl hover:text-primary-600 transition-all text-slate-400 shadow-sm"><Twitter size={16} /></a>}
                   {project.socials?.discord && <a href={project.socials.discord} target="_blank" className="p-2 bg-slate-50 dark:bg-slate-800 rounded-xl hover:text-primary-600 transition-all text-slate-400 shadow-sm"><DiscordIcon size={16} /></a>}
                </div>
              </div>
