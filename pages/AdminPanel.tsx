...
import {
   ...
   Layers, Search, Info, Megaphone, TrendingUp, ChevronRight, Lock, Clock, History, ChevronLeft, Wrench, RefreshCw
} from 'lucide-react';
...
export const AdminPanel: React.FC = () => {
   ...
   // NEW: State for Admin Auth
   const [isAuthenticated, setIsAuthenticated] = useState(false);
   const [adminPassword, setAdminPassword] = useState('');
   const [authError, setAuthError] = useState('');

   // EFFECT: Check if we have a valid session cookie/status
   useEffect(() => {
      // In a real app, we might check an endpoint /api/admin/check-session or trust that if user.isAdmin is true, 
      // we still want to force a re-login for the dashboard if 'sessionStorage' isn't set.
      // For this request, user wants a PASSWORD prompt. 
      // We will check if we have a session token.
      // But since we are client-side, we can just use a local state gate for now, backed by the API login.
   }, []);
   ...
   // --- EXISTING CRUD LOGIC WRAPPED WITH SUPABASE ---
   ...
   const syncToDb = async (table: string, data: any) => {
      // Remove UI-only fields if any, or spread safely
   ...
   const handleSave = async () => {
      const id = editingItem?.id || crypto.randomUUID(); // Use standard UUID
      ...
      <div className="max-w-[1600px] mx-auto pb-24 px-4">
         ...
         <div className="flex flex-col lg:flex-row gap-8">
            ...
            <main className="flex-1 min-w-0">
                        ...
                        {requests.map(r => (
                           <div key={r.id} className="p-6 bg-white dark:bg-slate-800 border dark:border-slate-700 rounded-[2rem] flex items-center justify-between group shadow-sm transition-all hover:border-primary-500">
                              <div>
                                 ...
                                 <div className="flex items-center gap-1 mt-1 text-[10px] font-bold text-slate-400 lowercase">
                                    <Twitter size={12} /> {r.twitterLink || 'No twitter provided'}
                                 </div>
                              </div>
                              <div className="flex gap-2">
               ...
               {activeTab === 'rpg' && (
                  <SectionWrapper title="RPG Activity Control" onAdd={() => openModal('rpg')}>
                     <div className="mb-8 flex items-center justify-between p-6 bg-rose-50 dark:bg-rose-950/20 rounded-3xl border border-rose-100 dark:border-rose-900/30">
                        <div>
                           <h4 className="font-black text-rose-600 dark:text-rose-400 uppercase tracking-tighter">Emergency XP Control</h4>
                           <p className="text-[10px] font-bold text-rose-500/70 uppercase">This will reset progress for all protocol units.</p>
                        </div>
                        <button onClick={() => { if (confirm("Are you sure you want to reset all user progress? This cannot be undone.")) { resetAllXPs(); addToast("All unit progress wiped."); } }} className="px-6 py-3 bg-rose-600 text-white rounded-2xl font-black text-xs uppercase tracking-widest shadow-xl active:scale-95 transition-all flex items-center gap-2">
                           <RefreshCw size={16} /> Reset All XP
                        </button>
                     </div>
                     <div className="space-y-3">{activities.filter(a => a.type === 'rpg').map(a => <ListItem key={a.id} title={a.name} sub={`Chain ${a.chainId} - Bonus XP: ${a.extraXP}`} img={a.logo} onEdit={() => openModal('rpg', a)} onDelete={() => setActivities(p => p.filter(x => x.id !== a.id))} />)}</div>
                  </SectionWrapper>
               )}
            </main>
         </div>
