-- 1. Create the Function to Handle New Messages (Updated for Premium Role)
...
BEGIN
  ...
  -- C. Broadcast to Premium Users Only
  ELSIF NEW."targetRole" = 'premium' THEN
    INSERT INTO public.inbox_messages ("userId", "title", "content", "type", "timestamp", "isRead")
    SELECT 
      id AS "userId",
      NEW.title, 
      ...
      false
    FROM public.users
    WHERE "memberStatus" IN ('Premium', 'Admin', 'Super Admin', 'Moderator');
  END IF;
